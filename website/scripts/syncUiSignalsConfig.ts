...
function parseSimpleYaml(source: string): YamlObject {
...
function sortDeep(value: YamlValue): YamlValue {
  ...
  if (value && typeof value === 'object') {
    const entries = Object.entries(value as YamlObject).sort(([a], [b]) => a.localeCompare(b));
    const sorted: YamlObject = {};
...
function computeHash(config: YamlObject): string {
  const canonical = JSON.stringify(sortDeep(config));
  return `sha256:${crypto.createHash('sha256').update(canonical, 'utf8').digest('hex')}`;
}

function generateOutput(config: YamlObject, hash: string): string {
