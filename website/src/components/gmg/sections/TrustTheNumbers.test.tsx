...
describe('TrustTheNumbers', () => {
  ...
  it('renders every concern in c.concerns.all, highest severity first', () => {
    const c = richTrust();
    expect(c.concerns.all.length).toBeGreaterThan(0);
    const { container } = render(<TrustTheNumbers c={c} p={p} isMobile={false} padX={16} />);
    for (const concern of c.concerns.all) {
      expect(container.textContent).toContain(concern.headline);
    }
    // The corpus-wide sort is high -> medium -> low; confirm this charity's
    // own list isn't accidentally re-sorted or reversed by the section.
    const severities = c.concerns.all.map((x) => x.severity);
