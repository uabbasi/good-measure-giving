...
export const LandingPage: React.FC = () => {
  ...
  return (
    ...
    <div className={`flex flex-col min-h-screen ${theme.bgPage} font-sans transition-colors duration-500`}>
      ...
      <section className={`relative pt-8 pb-10 md:pt-12 md:pb-14 lg:pt-16 lg:pb-20 overflow-hidden transition-colors duration-500 ${theme.bgHero}`}>
      ...
      <section className={`py-8 md:py-10 lg:py-16 relative z-10 overflow-hidden transition-colors duration-500 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'bg-slate-950' : 'bg-white border-y border-slate-100'}`}>
            ...
            <div className="lg:w-1/2 relative">
              ...
              <div className={`relative p-8 rounded-2xl shadow-2xl backdrop-blur-sm ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'bg-slate-800/90 border border-slate-600/50 ring-1 ring-white/10' : 'bg-white border border-slate-200 ring-1 ring-slate-100'}`}>
                ...
                <div className={`flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-8 pb-6 border-b ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'border-slate-700/50' : 'border-slate-100'}`}>
                  ...
                  <div className="flex-1">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 mb-3">
                      ...
                      <span className="text-xs font-bold text-emerald-500 uppercase tracking-wider">Featured Evaluation</span>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
                  <div className={`p-4 rounded-xl text-center border ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50 border-blue-100'}`}>
                    <div className={`text-xl font-bold ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-blue-400' : 'text-blue-600'}`}>{confidenceBadge}</div>
                    <div className={`text-xs font-medium mt-1 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-400' : 'text-slate-500'}`}>Data Confidence</div>
                    <div className={`text-[10px] mt-0.5 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-500' : 'text-slate-400'}`}>Can we verify claims?</div>
                  </div>
                  <div className={`p-4 rounded-xl text-center border ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-50 border-emerald-100'}`}>
                    <div className={`text-xl font-bold ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-emerald-400' : 'text-emerald-600'}`}>{riskLevel}</div>
                    <div className={`text-xs font-medium mt-1 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-400' : 'text-slate-500'}`}>Risk Check</div>
                    <div className={`text-[10px] mt-0.5 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-500' : 'text-slate-400'}`}>Major concerns flagged?</div>
                  </div>
                  <div className={`p-4 rounded-xl text-center border ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'bg-purple-500/10 border-purple-500/20' : 'bg-purple-50 border-purple-100'}`}>
                    <div className={`text-xl font-bold ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-purple-400' : 'text-purple-600'}`}>{walletLabel}</div>
                    <div className={`text-xs font-medium mt-1 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-400' : 'text-slate-500'}`}>Giving Route</div>
                    <div className={`text-[10px] mt-0.5 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-500' : 'text-slate-400'}`}>Zakat or Sadaqah guidance</div>
                  </div>
                </div>
                ...
                {scoreSummary && (
                  <div className={`p-5 rounded-xl mb-6 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'bg-slate-900/80 border border-slate-700/50' : 'bg-slate-50 border border-slate-200'}`}>
                    <div className={`text-xs font-bold uppercase tracking-wider mb-2 ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-400' : 'text-slate-500'}`}>Evaluation Summary</div>
                    <p className={`text-sm leading-relaxed ${theme.id.includes('dark') || theme.id === 'warm-atmosphere' ? 'text-slate-300' : 'text-slate-700'}`}>
                      {scoreSummary}
                    </p>
                  </div>
                )}
