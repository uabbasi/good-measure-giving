  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <section className="mb-20">
          <h2 className={`text-2xl font-bold font-merriweather mb-4 [text-wrap:balance] ${isDark ? 'text-white' : 'text-slate-900'}`}>Reference &amp; Citation</h2>
          <p className={`mb-8 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
            The exact formula, current rubric version, and source data behind every score {'—'} so you can
            reference our methodology directly.
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            ...
            <div className={`rounded-2xl border p-6 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>The Scoring Formula</h3>
              </div>
              <div className={`rounded-lg p-4 font-mono text-sm ${isDark ? 'bg-slate-800 text-slate-200' : 'bg-slate-100 text-slate-800'}`}>
                GMG Score = Impact + Alignment {'−'} Risk
              </div>
              <ul className={`text-sm space-y-1 mt-4 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                <li>{'•'} <strong>Impact</strong>: 0 to 50 points</li>
                <li>{'•'} <strong>Alignment</strong>: 0 to 50 points</li>
                <li>{'•'} <strong>Risk</strong>: 0 to {'−'}10 points (deductions)</li>
                <li>{'•'} <strong>Total</strong>: 0 to 100</li>
              </ul>
            ...
            <div className={`rounded-2xl border p-6 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              <div className="flex items-center gap-2 mb-4">
                <FileText className={`w-5 h-5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} aria-hidden="true" />
                <h3 className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Version &amp; Sources</h3>
              </div>
              <dl className="text-sm space-y-3">
                <div className="flex items-center justify-between gap-4">
                  <dt className={isDark ? 'text-slate-400' : 'text-slate-600'}>Rubric version</dt>
                  <dd>
                    <span className={`px-2 py-0.5 rounded font-mono text-xs ${isDark ? 'bg-emerald-900/30 text-emerald-300' : 'bg-emerald-100 text-emerald-800'}`}>v{RUBRIC_VERSION}</span>
                  </dd>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <dt className={isDark ? 'text-slate-400' : 'text-slate-600'}>Last updated</dt>
                  <dd className={isDark ? 'text-slate-200' : 'text-slate-800'}>{METHODOLOGY_LAST_UPDATED}</dd>
                </div>
                <div>
                  <dt className={`mb-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Data sources</dt>
                  <dd className={`text-xs leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                    IRS Form 990 (via ProPublica), Charity Navigator, Candid (GuideStar), BBB Wise Giving
                    Alliance, charity websites, and web search.
                  </dd>
                </div>
              </dl>
            </div>
          ...
          <div className={`rounded-2xl border p-6 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            <div className="flex items-center justify-between gap-4 mb-3">
              <div className="flex items-center gap-2">
                <Quote className={`w-5 h-5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} aria-hidden="true" />
                <h3 className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Cite This Page</h3>
              </div>
              <button
                type="button"
                onClick={copyCitation}
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${isDark ? 'bg-slate-700 text-slate-200 hover:bg-slate-600' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}
              >
                {citationCopied ? <Check className="w-3.5 h-3.5" aria-hidden="true" /> : <Copy className="w-3.5 h-3.5" aria-hidden="true" />}
                {citationCopied ? 'Copied' : 'Copy'}
              </button>
            </div>
            <p className={`text-sm mb-3 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              Referencing our scoring in research, a grant report, or an article? Use this citation:
            </p>
            <p className={`rounded-lg p-4 text-sm leading-relaxed ${isDark ? 'bg-slate-800 text-slate-200' : 'bg-slate-100 text-slate-800'}`}>
              {CITATION_TEXT}
            </p>
            <p className={`text-xs mt-4 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
              Run a charity yourself? Add our trust badge or a backlink from our{' '}
              <Link to="/link-to-us" className={isDark ? 'text-emerald-400 hover:text-emerald-300 underline' : 'text-emerald-600 hover:text-emerald-700 underline'}>
                Link to Us
              </Link>{' '}
              page.
            </p>
